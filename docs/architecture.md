...
| 模块 | 目录 | 职责 |
|---|---|---|
| 前端应用 | `frontend/` | 用户入口、推荐展示、电影分页浏览、跳转页码、评分提交与修改、LLM 问答 |
...
| `RatingService` | 校验并保存/更新评分，查询用户评分列表，触发推荐更新 |
...
前端已经从单一主界面调整为菜单式应用，包含“推荐结果”“电影库与评分”“我的评分”“数据看板”“智能问答”五个主要视图。其中“我的评分”作为独立菜单页展示，不再嵌入评分栏下方。
